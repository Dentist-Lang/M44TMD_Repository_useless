...
writer = open("log_M44TMD_ResNet50_dec-fus.txt", "w+")
...
root_folder = "/content"
data_folder = "/content/ZJU-H2 TMJ MRI Dataset/MRI Data"
label_folder = "/content/ZJU-H2 TMJ MRI Dataset/Annotations"
...
class MultiTaskModel(nn.Module):
    def __init__(self, freeze_layers_num=0):
        ...
        self.task1_img_classifier = nn.Linear(2048 * 3, 2)
        self.task2_img_classifier = nn.Linear(2048 * 3, 3)
        self.task3_img_classifier = nn.Linear(2048 * 3, 2)

        self.task1_clinical_classifier = nn.Linear(256, 2)
        self.task2_clinical_classifier = nn.Linear(256, 3)
        self.task3_clinical_classifier = nn.Linear(256, 2)
    ...
    def forward(self, closed_pd_images, closed_t2w_images, open_pd_images, clinical_data):
        ...
        combined_image_feature = torch.cat([closed_pd_features, closed_t2w_features, open_pd_features], dim=1)
        combined_image_feature = combined_image_feature.view(combined_image_feature.size(0), -1)

        clinical_feature = self.clinical_features(clinical_data.to(device))

        task1_img_output = self.task1_img_classifier(combined_image_feature)
        task2_img_output = self.task2_img_classifier(combined_image_feature)
        task3_img_output = self.task3_img_classifier(combined_image_feature)

        task1_clinical_output = self.task1_clinical_classifier(clinical_feature)
        task2_clinical_output = self.task2_clinical_classifier(clinical_feature)
        task3_clinical_output = self.task3_clinical_classifier(clinical_feature)

        # decision fusion
        task1_output = task1_img_output + task1_clinical_output
        task2_output = task2_img_output + task2_clinical_output
        task3_output = task3_img_output + task3_clinical_output
...
figures_folder = "./figures/M44TMD_ResNet50_dec-fus"
...
def plot_roc_curve(test_labels_binarized, preds_binarized, task_name, num_classes, epoch):
    ...
    plt.savefig(os.path.join(figures_folder, f"M44TMD_ResNet50_dec-fus_{task_name}_epoch_{epoch}_roc_curve.png"))
    plt.close()
...
def train_and_evaluate(freeze_layers_num):
    ...
    checkpoint_path = os.path.join(checkpoint_folder, "M44TMD_ResNet50_dec-fus.pth")
